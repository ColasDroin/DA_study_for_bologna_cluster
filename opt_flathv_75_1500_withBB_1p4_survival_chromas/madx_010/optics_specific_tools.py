...
def build_sequence(mad, beam, configuration):
    ...
    pm.make_links({"optics_toolkit": f"optics_repository/toolkit"}, force=True)
...
def twiss_and_check(
    ...
    if check_betas_at_ips:
        for ss in sequences_to_check:
            tt = twiss_dfs[ss]
            _check_beta_at_ips_against_madvars(
                beam=ss[-1], twiss_df=tt, variable_dicts=var_dict, tol=tol_beta
            )
    ...
    if check_separations_at_ips:
        ...
        _check_separations_at_ips_against_madvars(twiss_df_b1, twiss_df_b2, var_dict, tol=tol_sep)
...
def lumi_control(mad, twiss_dfs, configuration, knob_names):
    ...
    def function_to_minimize_ip8(sep8_m):
        my_dict_IP8 = pm.get_luminosity_dict(mad, twiss_dfs, "ip8", configuration["nco_IP8"])
    ...
    sigma_sep_b1_ip8 = np.sqrt(
        twiss_dfs["lhcb1"].loc["ip8:1"]["bet" + sep_plane_ip8]
    ...
    mad.globals[sep_knobname_ip2] = (
        np.sign(mad.globals[sep_knobname_ip2])
        * configuration["fullsep_in_sigmas_ip2"]
        * sigma_sep_b1_ip2
        / 2
        * 1e3
    )
...
def _check_separations_at_ips_against_madvars(twiss_df_b1, twiss_df_b2, variables_dict, tol):
...
def filter_bb_df(bb_df, bb_schedule_to_track):
    ...
    lr_ATLAS = lr_ATLAS[
        lr_ATLAS["identifier"].apply(lambda x: x in bb_schedule_to_track["Positions in ATLAS/CMS"])
    ]

    # BBLR ALICE
    lr_ALICE = bb_df[(bb_df["ip_name"] == "ip2") & (bb_df["label"] == "bb_lr")].copy()
    lr_ALICE = lr_ALICE[
        lr_ALICE["identifier"].apply(lambda x: x in bb_schedule_to_track["Positions in ALICE"])
    ]

    # BBLR CMS
    lr_CMS = bb_df[(bb_df["ip_name"] == "ip5") & (bb_df["label"] == "bb_lr")].copy()
    lr_CMS = lr_CMS[
        lr_CMS["identifier"].apply(lambda x: x in bb_schedule_to_track["Positions in ATLAS/CMS"])
    ]

    # BBLR LHCB
    lr_LHCB = bb_df[(bb_df["ip_name"] == "ip8") & (bb_df["label"] == "bb_lr")].copy()
    lr_LHCB = lr_LHCB[
        lr_LHCB["identifier"].apply(lambda x: x in bb_schedule_to_track["Positions in LHCB"])
    ]
